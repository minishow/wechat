$(function(){
	$("#table_datagrid").datagrid({
		...
		fitColumns:true,
	    columns:[[       
	              {field:'name',title:'规格名称',width:100,align:'center'},    
...
function myAdd(){
	/*点击添加的时候,给表格添加1行,并全部设置为空,地点信息从productScale查询出来*/
...
function myDelete(){
	...
	if(mySelected){
		$.messager.confirm("温馨提示","确定要修改这条数据的状态吗?",function(flag){
			if(flag){
				$.get("/productScale/delete?id="+mySelected.id,function(data){
					if(data.success){
						//刷新表格
						$("#table_datagrid").datagrid("reload");
					}
